  ...
  methods: {
    ...
    beforeRead(file) {
      ...
      fileList.forEach(f => {
        ...
        console.log('f.name: ', fileName, fileName.toLowerCase().split('.').pop());
        const checkExtension = () =>
          !this.extensionList.length || this.extensionList.includes(fileName.toLowerCase().split('.').pop());
        if (!checkExtension()) {
    ...
    async handleDelete(val) {
      ...
      const { attachId, attachName, attachSize, attachUrl, fileType, id } = val;
      const params = [
        {
          attachId,
          attachName,
      ...
      const {
        data: { code, msg },
      } = await request.post(deleteBatch, params);
      this.$toast.clear();
